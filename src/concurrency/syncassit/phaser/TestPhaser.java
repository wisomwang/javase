...
public class TestPhaser {
	...
	public static void main(String[] args) {
		Phaser phaser = new Phaser(3) {// 共有3个工作线程，因此在构造函数中赋值为3
			...
			/** 每一阶段执行结束后，会调用这个方法，如果方法返回为true,表示终结 */
			@Override
		...
		for (int i = 0; i < 3; i++) { // 创建并启动3个线程
			new MyThreadOne((char) (97 + i), phaser).start();
		}

		phaser.register(); // 将主线程动态增加到phaser中，此句执行后phaser共管理4个线程

		/** 这段代码类似于Test类中的把三个线程调用join方法 */
		while (!phaser.isTerminated()) {// 只要phaser不终结，主线程就循环等待
			phaser.arriveAndAwaitAdvance(); // 主线程也要调用该方法表示处于等待，不然其他线程无法进入下一阶段
		}
...
class MyThreadOne extends Thread {
	...
	public MyThreadOne(char c, Phaser phaser) {
		this.c = c;
		this.phaser = phaser;
	}
